...
# Specify the (base) font size for all plots
fs = 7
...
ggplot() +
  ...
  theme_linedraw(base_family = "Times", base_size = f1_fs) +
  theme(
...
ggplot() +
  ...
  theme_linedraw(base_family = "Times", base_size = f2_fs) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.text.x = element_blank(),
...
sine_plot_obs <- ggplot(sine_data_train, aes(x = x1, y = x2, color = y)) +
  ...
  theme_bw(base_family = "Times", base_size = f3_fs) +
...
if (!file.exists("sine_lime_explain.rds")) {
    ...
    saveRDS(sine_lime_explain, "sine_lime_explain.rds")

  } else {

    sine_lime_explain <- readRDS("sine_lime_explain.rds")
...
sine_poi_exp <- 
  ...
  plot_features() + 
  theme(text = element_text(family = "Times", size = f4_fs),
...
exp_scatter <-
  ...
  theme_bw(base_family = "Times", base_size = f4_fs) +
  guides(shape = guide_legend(order = 1),
         size = guide_legend(
           nrow = 2,
           byrow = T,
...
heatmap_plot_good <- ggplot(data = heatmap_data_good,
  ...
  theme_bw(base_family = "Times", base_size = f5_fs) +
  scale_fill_grey() +
  scale_color_grey() +
...
plot_feature_heatmap(sine_lime_explain$explain,
                     order_method = "PCA") +
  theme_bw(base_family = "Times", base_size = f6_fs) +
  theme(
...
plot_metrics(sine_lime_explain$explain, rank_legend = "discrete") +
  theme_bw(base_family = "Times", base_size = f7_fs) +
  theme(
    strip.background = element_rect(color = "white", fill = "white"),
    strip.placement = "outside",
...
# Load the hamby data
bullet_train <- read.csv("hamby173and252_train.csv")
bullet_test <- read.csv("hamby224_test.csv")
...
## ----figure-09, out.width = '3.125in', fig.width = 6, fig.height = 3, warning = FALSE----
...
signatures <- readRDS("signatures.rds")
...
signatures %>%
  ...
  facet_grid(land~.) +
  ylim(c(-4,6)) +
  theme_bw(base_family = "Times", base_size = f9_fs) +
...
if (!file.exists("figure-10-1.png")) {
  ...
  bullet_pcp <- bullet_train %>%
    ...
    scale_color_gradient2(low = "grey50",
                          high = "darkorange",
                          midpoint = 0.5) +
    theme_bw(base_family = "Times", base_size = f10_fs) +
    theme(
...
# Create (or load) the parallel coordinate plot
if (file.exists("figure-10-1.eps")) {
...
if (file.exists("hamby_lime.rds")){

  # Load the files
  bullet_explain_noperms <- readRDS("hamby_explain.rds")
...
plot_feature_heatmap(
  ...
  scale_color_gretchenalbrecht(palette = "last_rays",
                               discrete = TRUE,
                               reverse = TRUE) +
  theme_bw(base_family = "Times", base_size = f11_fs) +
  theme(
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    strip.background = element_rect(color = "white", fill = "white"),
...
## ----figure-12, out.width = '6.5in', warning = FALSE, fig.width = 10, fig.height = 4----
...
plot_metrics(
  ...
  theme_bw(base_family = "Times", base_size = f12_fs) +
  theme(
    strip.background = element_rect(color = "white", fill = "white"),
    strip.placement = "outside",
...
## ----figure-13, out.width = '6.25in', fig.width = 13, fig.height = 17, message = FALSE----
...
# Create the explanation scatterplot: case M, 3 quantile bins
eoiM_3qb <-
  plot_explain_scatter(
    bullet_explain_perms[1:3,],
    alpha = 0.9,
    weights = TRUE,
    line_size = f13_ls
  ) +
  ...
  theme_bw(base_family = "Times", base_size = f13_fs) +
  theme(
    aspect.ratio = 1,
...
# Create the explanation scatterplot: case MM, 3 quantile bins
eoiNM_3qb <-
  plot_explain_scatter(
    bullet_explain_perms[4:6, ],
    alpha = 0.9,
    weights = TRUE,
    line_size = f13_ls
  ) +
  ...
  guides(linetype = guide_legend(override.aes = list(color = c(
    "grey50", "darkorange"
  ))),
  size = guide_legend(nrow = 2, byrow = T))

# Create the explanation scatterplot: case M, 3 equal bins
eoiM_3eb <-
  plot_explain_scatter(
    bullet_explain_perms[7:9,],
    alpha = 0.9,
    weights = TRUE,
    line_size = f13_ls
  ) +
...
# Create the explanation scatterplot: case NM, 3 equal bins
eoiNM_3eb <-
  plot_explain_scatter(
    bullet_explain_perms[10:12,],
    alpha = 0.9,
    weights = TRUE,
    line_size = f13_ls
  ) +
...
# Create the lime explanation visualization: case M, 3 quantile bins
pfM_3qb <- plot_features(bullet_explain_perms[1:3,]) +
  scale_fill_manual(values = c("darkorange", "grey50")) +
  theme(text = element_text(family = "Times", size = f13_fs),
        strip.text.x = element_text(face = "plain", size = f13_fs))

# Create the lime explanation visualization: case NM, 3 quantile bins
pfNM_3qb <- plot_features(bullet_explain_perms[4:6,]) +
  scale_fill_manual(values = c("darkorange", "grey50")) + 
  theme(text = element_text(family = "Times", size = f13_fs),
        strip.text.x = element_text(face = "plain", size = f13_fs))

# Create the lime explanation visualization: case M, 3 equal bins
pfM_3eb <- plot_features(bullet_explain_perms[7:9,]) +
  scale_fill_manual(values = c("darkorange", "grey50")) +
  theme(text = element_text(family = "Times", size = f13_fs),
        strip.text.x = element_text(face = "plain", size = f13_fs))

# Create the lime explanation visualization: case NM, 3 equal bins
pfNM_3eb <- plot_features(bullet_explain_perms[10:12,]) +
  scale_fill_manual(values = c("darkorange", "grey50")) +
  theme(text = element_text(family = "Times", size = f13_fs),
        strip.text.x = element_text(face = "plain", size = f13_fs))
...
plot_explain_scatter(
  ...
  theme_bw(base_family = "Times", base_size = fb1_fs) +
  theme(
...
bullet_es_M <-
  ...
  theme_bw(base_family = "Times", base_size = fb2_fs) +
  theme(
    aspect.ratio = 1,
    strip.background = element_rect(fill = "white", color = "white"),
    strip.placement = "outside",
...
ggplot(sine_poi_perms, aes(x = exppred, y = resid)) +
  ...
  theme_bw(base_family = "Times", base_size = fc3_fs) +
