...
The random forest model misclassifies \Sexpr{dim(sine_data_test %>% filter(y != rfpred))[1]} points on the classification boundary. These are identified by circles in \autoref{fig:sine-plot-data}. For the visualizations of the steps of the LIME algorithm, we focus on one of these points: the observation with $(x_1, x_2, x_3)$ coordinates of (\Sexpr{round(sine_poi$x1, 2)}, \Sexpr{round(sine_poi$x2, 2)}, \Sexpr{round(sine_poi$x3, 2)}) and indicated in \autoref{fig:sine-plot-data} by a diamond.
...
\begin{figure}[!htbp]
\centering
<<sine-plot-poi, out.width = '0.5\\textwidth', fig.width = 4, fig.height = 3.25>>=
sine_poi_exp <- sine_poi_lime_default %>% plot_features()
sine_poi_exp
@
\caption{Visualization from \emph{lime} of the explanation associated with the prediction of interest from the \data \ produced by the default settings. The probability of \Sexpr{sine_poi_lime_default$label_prob[1]} is the random forest probability in support of a classification of blue. The colored bars indicate that $\Sexpr{str_split(sine_poi_lime_default$feature_desc[[1]], pattern = " ")[[1]][1]} < x_2$ supports a random forest prediction of 1 and $\Sexpr{str_split(sine_poi_lime_default$feature_desc[[2]], pattern = " ")[[1]][1]}<x_1\le \Sexpr{str_split(sine_poi_lime_default$feature_desc[[2]], pattern = " ")[[1]][5]}$ contradicts a random forest prediction of 0. The explanation fit of \Sexpr{round(sine_poi_lime_default$model_r2[1], 2)} is the $R^2$ value of the explainer model, which indicates that the explainer model has a poor linear fit. Aside of some evidence from the $R^2$ value, it is not clear whether this explanation is reasonable.}
\label{fig:sine-plot-poi}
\end{figure}

\begin{figure*}[!htbp]
\centering
<<sine-plot-step1ab, out.width = '\\textwidth', fig.width = 8.25, fig.height = 3.25, warning = FALSE>>=
...
# Join the plots
plot_grid(sine_plot_bins, sine_plot_sims,
          nrow = 1,
...
\begin{figure}[!htbp]
\centering
<<sine-plot-step1c, out.width = '0.5\\textwidth', fig.width = 4.25, fig.height = 3.25, warning = FALSE>>=
...
Overlaying the simulated data with the interpretability transformed values highlights the simplification imposed by the transformation. The rectangle where the prediction of interest is located contains a wide range of random forest predictions from \Sexpr{sine_poi_perms %>% filter(x1num == 1, x2num == 1) %>% summarise(min(rfpred)) %>% round(2)} to \Sexpr{sine_poi_perms %>% filter(x1num == 1, x2num == 1) %>% summarise(max(rfpred)) %>% round(2)}, but the interpretability transformation groups all of these values into one category. While this transformation  provides easily understandable interpretations, it leads to an extreme over-simplification of the random forest model. This simplification may be one of the reasons underlying the low $R^2$ value associated with the final explainer model and provides further evidence against the LIME claim of faithfulness for this explainer model.
...
\begin{figure*}[!htp]
\centering
<<sine-plot-step2ab, out.width = '\\textwidth', fig.width = 8, fig.height = 3.25>>=
...
plot_grid(step2a, step2b, rel_widths = c(0.55, 0.45))
@
...
\begin{figure*}[!htbp]
<<sine-plot-step2cd, out.width = '\\textwidth', fig.width = 8, fig.height = 3.25, warning = FALSE>>=
...
step2c <- ggplot() +
  ...
  labs(x = TeX("x_1"),
       y = TeX("x_2"),
       fill = "Explainer \nModel \nPrediction \nfor 'blue'",
       title = "Explainer Model Prediction Plot",
       shape = "")
...
step2d <- ggplot(sine_poi_perms, aes(x = exppred, y = resid)) +
...
plot_grid(step2c, step2d, rel_widths = c(0.51, 0.49))
@
...
Compared to the relationship between the random forest predictions and the features of $x_1$ and $x_2$ shown in Figure \ref{fig:sine-plot-step1ab}, the explainer model is a major simplification of the random forest model. Consider the region where the prediction of interest is located. The explainer model provides a single prediction of \Sexpr{round(sine_poi_lime_default$model_prediction[[1]],2)} for the entire region as a summary of random forest predictions ranging from \Sexpr{round(min(sine_poi_region_perms$rfpred), 2)} to \Sexpr{round(max(sine_poi_region_perms$rfpred), 2)} with a mean and standard deviation \Sexpr{round(mean(sine_poi_region_perms$rfpred), 2)} and \Sexpr{round(sd(sine_poi_region_perms$rfpred), 2)}, respectively. The random forest prediction for the prediction of interest is \Sexpr{round(sine_poi$rfprob_blue,2)}. Consider that the mean prediction from the random forest (\Sexpr{round(mean(sine_poi_region_perms$rfpred), 2)}) agrees with the explainer model prediction (\Sexpr{round(sine_poi_lime_default$model_prediction[[1]],2)}) as is expected due to the nature of the explainer model used, but the random forest prediction associated with the case of interest (\Sexpr{round(sine_poi$rfprob_blue,2)}) is about a half \sout{1} standard deviation away from the mean. \kgc{After reading the previous sentence again, I'm wondering if the next sentence is too dramatic. (Especially since I had made a mistake with the standard deviation.)} \hh{XXX how about we put a 'suggest' into it?} The difference between the explainer model and random forest predictions for the case of interest along with the amount of variability in the random forest predictions in this region makes it clear that the explainer model is simplifying the random forest model too much and violating the assumption of faithfulness. 
...
\begin{figure*}[!htp]
<<sine-plot-step3ab, out.width = '\\textwidth', fig.width = 10, fig.height = 4, warning = FALSE>>=

# My lime explanation plot
step3a <- sine_poi_lime_default %>% 
...
We can use the explanation plot to interpret the explainer model in terms of a ridge regression coefficients. The intercept, \Sexpr{round(sine_poi_lime_default$model_intercept[1], 2)}, is the predicted random forest probability for observations that are not in the same $x_1$ or $x_2$ quantile bins as the prediction of interest. The coefficients are interpreted as the amount that the average random forest probability increases/decreases if an observation falls in the same bin as the prediction of interest. In this example, the average random forest probability that an observation is in category 'blue' increases by \Sexpr{sine_poi_lime_default %>% filter(feature == "x2") %>% select(feature_weight) %>% round(2)} if an observation falls in the same $x_2$ quantile bin as the prediction of interest (i.e. the third quantile), and the average random forest probability that an observation is in category 'blue' decreases by \Sexpr{sine_poi_lime_default %>% filter(feature == "x1") %>% select(feature_weight) %>% round(2) %>% abs()} if an observation falls in the same $x_1$ quantile bin as the prediction of interest (i.e. the fourth quantile). Because the magnitude of the coefficient associated with $x_2$ is larger, it indicates that $x_2$ affects the random forest prediction more than $x_1$. Note that if a different simulation method is used when applying LIME, the interpretation of the explainer model can change. See Appendix \ref{lime} for more details.
...
%\sout{In the case of the prediction of interest from the \data, we can interpret the explainer model as follows. \hh{XXX Instead of writing out numbers, I would suggest to describe the bins in words lowest quartile, third, second, top quartile, ...  XXX} For an observation that has a value of $x_1$ that is greater than \Sexpr{round(x1_lower, 1)} and less than or equal to \Sexpr{round(x1_upper, 1)}, the probability that the observation is predicted to be in category blue decreases by \Sexpr{abs(round(sine_b1, 1))}, and if an observation has a value of $x_2$ greater than \Sexpr{round(x2_lower, 1)}, the probability that the observation is predicted to be in category blue increases by \Sexpr{abs(round(sine_b2, 1))}. Since the magnitude of the coefficient associated with $x_2$ is larger, it indicates that $x_2$ plays a more important role of the random forest prediction than $x_1$. The figure produced by \emph{lime} shown in} \autoref{fig:sine-plot-poi} \sout{shows a visualization of the coefficients from the explainer model to provide a high level visualization of the LIME explanation.}
...
\begin{figure*}[!htbp]
<<sine-plot-selected-features, out.width = "\\textwidth", fig.width = 8, fig.height = 3.25>>=
...
\begin{figure*}[!htbp]
<<sine-plot-agg-exp, warning = FALSE, out.width = '\\textwidth', fig.width = 10, fig.height = 3.25>>=
...
\begin{figure}[!t]
<<sine-plot-grid, warning = FALSE, out.width = '0.5\\textwidth', fig.width = 4, fig.height = 3.2>>=
...
\hh{is} $x^*=$ (\Sexpr{round(sine_poi$x1, 2)}, \Sexpr{round(sine_poi$x2, 2)}, \Sexpr{round(sine_poi$x3, 2)}) \hh{with} $f(x^*)=0.83$.
...
\begin{figure}[!t]
<<bullet-plot-train, out.width = '0.5\\textwidth', fig.width = 5, fig.height = 5, warning = FALSE>>=
bullet_train %>%
...
\begin{figure}[!t]
<<bullet-plot-scatter, out.width = '0.5\\textwidth', fig.width = 9, fig.height = 9, warning = FALSE>>=
...
\begin{figure}[!b]
<<bullet-plot-rfvi, out.width = '0.5\\textwidth', fig.width = 3.5, fig.height = 1.5>>=
...
\begin{figure*}[!t]
<<bullet-lime-expls, out.width = '\\textwidth', warning = FALSE, fig.width = 12, fig.height = 5.5>>=

bullet_poi_nonmatch_exps <- bullet_explain %>%
  filter(sim_method == "quantile_bins",
         gower_pow == 1,
         case == bullet_poi_nonmatch) %>%
  mutate(case = paste0(case, ", ", nbins, " Quantile Bins"))


nb2 <- plot_features(bullet_poi_nonmatch_exps %>% filter(nbins == 2)) +
  scale_fill_manual(values = c("darkorange", "grey50"))
nb3 <- plot_features(bullet_poi_nonmatch_exps %>% filter(nbins == 3)) +
  scale_fill_manual(values = c("darkorange", "grey50"))
nb4 <- plot_features(bullet_poi_nonmatch_exps%>% filter(nbins == 4)) +
  scale_fill_manual(values = c("darkorange", "grey50"))
nb5 <- plot_features(bullet_poi_nonmatch_exps %>% filter(nbins == 5)) +
  scale_fill_manual(values = c("darkorange", "grey50"))
nb6 <- plot_features(bullet_poi_nonmatch_exps %>% filter(nbins == 6)) +
  scale_fill_manual(values = c("darkorange", "grey50"))

feat_ord <- bullet_poi_nonmatch_exps %>%
  count(feature) %>%
  arrange(n) %>%
  pull(feature)

bc <- bullet_poi_nonmatch_exps %>%
  mutate(feature = factor(feature, levels = feat_ord)) %>%
  mutate(feature = fct_recode(feature,
         "Rough Correlation" = "rough_cor",
         "Non-CMS" = "non_cms",
  ...
  labs(x = "Feature",
       ...
       title = "Features Selected by LIME \nfor 2 to 6 Quantile Bins")

plot_grid(nb2, nb3, nb4, nb5, nb6, bc, nrow = 2)
@
...
$$g\left(\textbf{Z}'_{2}\right) = \Sexpr{round(sine_b0, 1)}\Sexpr{round(sine_b1, 1)}I\left[x_1\in (\Sexpr{round(x1_lower, 1)}, \Sexpr{round(x1_upper, 1)}] \right]+\Sexpr{round(sine_b2, 1)}I\left[x_2\in (\Sexpr{round(x2_lower, 1)}, \Sexpr{round(x2_upper, 1)})\right].$$
...
\begin{figure*}[!t]
<<app-plot-step0, out.width = "\\textwidth", fig.width = 8, fig.height = 4.4>>=

# Scatterplots of training data
x1x2 <- ggplot(sine_data_train, aes(x = x1, y = x2, color = y)) +
  geom_point(alpha = 0.8) +
  scale_colour_manual(values = c(sinecolor_blue, sinecolor_red)) +
  theme_bw(base_family = "Times", base_size = 10) +
  labs(x = TeX("$x_1$"),
...
x1x3 <- ggplot(sine_data_train, aes(x = x1, y = x3, color = y)) +
  geom_point(alpha = 0.8) +
  scale_colour_manual(values = c(sinecolor_blue, sinecolor_red)) +
  theme_bw(base_family = "Times", base_size = 10) +
  labs(x = TeX("$x_1$"),
       ...
       title = "") +
  theme(legend.position = "none")
x2x3 <- ggplot(sine_data_train, aes(x = x2, y = x3, color = y)) +
  geom_point(alpha = 0.8) +
  scale_colour_manual(values = c(sinecolor_blue, sinecolor_red)) +
  theme_bw(base_family = "Times", base_size = 10) +
  labs(x = TeX("$x_2$"),
...
train_legend <- get_legend(x2x3)
x2x3 <- x2x3 + theme(legend.position = "none")
...
x2x3test <- ggplot() +
  ...
  guides(shape = guide_legend(order = 1)) +
  theme(legend.title = element_text(size = 8))
test_legend <- get_legend(x2x3test)
x2x3test <- x2x3test + theme(legend.position = "none")

# Join the plots
plot_grid(x1x2, x1x3, x2x3, train_legend,
          x1x2test, x1x3test, x2x3test, test_legend,
          nrow = 2,
...
\begin{figure*}[!t]
\centering
<<app-plot-step1a, out.width = '\\textwidth', fig.width = 8, fig.height = 4.4>>=
...
bins_x1x2 <- ggplot() +
  ...
  labs(x = TeX("$x_1$"),
       ...
       fill = TeX("y"),
       shape = "",
       title = "Training Data with Quantile Bins") +
  guides(shape = guide_legend(order = 1),
         fill = FALSE) +
  theme(legend.position = "none",
        plot.title = element_text(size = 10))
...
bins_leg <- get_legend(bins_x2x3)
bins_x2x3 <- bins_x2x3 + theme(legend.position = "none")
...
bins_sim_x1x2 <- ggplot() +
  ...
  labs(x = TeX("$x'_1$"),
       ...
       color = "Random \nForest \nProbability",
       shape = "",
       title = "Quantile Bin Simulated Data") +
  guides(shape = guide_legend(order = 1)) +
  theme(legend.position = "none",
        plot.title = element_text(size = 10))
...
plot_grid(bins_x1x2, bins_x1x3, bins_x2x3, bins_leg,
          bins_sim_x1x2, bins_sim_x1x3, bins_sim_x2x3, bins_sim_leg,
          nrow = 2,
          rel_widths = c(0.29, 0.29, 0.29, 0.13))
...
\begin{figure*}[!t]
\centering
<<app-plot-step1b, out.width = '\\textwidth', fig.width = 14, fig.height = 4>>=
...
num_x1x2 <- ggplot() +
  ...
  labs(x = TeX("$x'_1$"),
       y = TeX("$x'_2$"),
       fill = "Numerified \nFeature Value",
       title = "Interpretability Transformed Simulated Data",
       shape = "") +
  guides(shape = FALSE,
         size = FALSE,
         color = FALSE)

x3_min <- min(sine_poi_perms$x3)
x3_upper <- sine_lime_default_bin_cuts$x3[[2]]
x3_max <- max(sine_poi_perms$x3)
...
rect_x1x3 <-
  data.frame(rect = factor(1:6),
             ...
             x1 = c(-10, x1_lower, x1_upper, -10, x1_lower, x1_upper),
             x2 = c(x1_lower, x1_upper, 10, x1_lower, x1_upper, 10),
             y1 = c(x3_min, x3_min, x3_min,
                    x3_upper, x3_upper, x3_upper),
             y2 = c(x3_upper, x3_upper, x3_upper,
...
num_x1x3 <- ggplot() +
  ...
  geom_rect(data = rect_x1x3,
            ...
            color = "black") +
  geom_point(data = sine_poi,
             mapping = aes(x = x1, y = x3, color = rfprob_blue),
             size = 5,
             shape = 18,
             alpha = 0.8) +
  geom_point(data = sine_poi %>% mutate(shape = "Prediction \nof Interest"),
...
# Create a dataset with the rectangles that make up the four categories
rect_x2x3 <-
  data.frame(rect = factor(1:4),
             z1 = c(0, 1, 0, 1),
             z2 = c(0, 0, 1, 1),
             case = factor(c(00, 10, 01, 11)),
             x1 = c(-10, x2_lower, -10, x2_lower),
             x2 = c(x2_lower, 10, x2_lower, 10),
             y1 = c(x3_upper, x3_upper, x3_min, x3_min),
             y2 = c(x3_max, x3_max, x3_upper, x3_upper)) %>%
  mutate(empred = sine_explainer(z1, z2),
         case_names = ifelse(case == "11", "\nz'2 = 1 \nz'3 = 1\n",
                      ifelse(case == "10", "\nz'2 = 1 \nz'3 = 0\n",
                      ifelse(case == "1", "\nz'2 = 0 \nz'3 = 1\n",
...
num_x2x3 <- ggplot() +
  ...
  geom_rect(data = rect_x2x3,
            ...
            color = "black") +
  geom_point(data = sine_poi,
             mapping = aes(x = x2, y = x3, color = rfprob_blue),
             size = 5,
             shape = 18,
             alpha = 0.8) +
  geom_point(data = sine_poi %>% mutate(shape = "Prediction \nof Interest"),
...
num_legend <- get_legend(num_x2x3 + guides(fill = FALSE))
num_x2x3 <- num_x2x3 + guides(shape = FALSE)

plot_grid(num_x1x2, num_x1x3, num_x2x3, nrow = 1) %>%
  plot_grid(num_legend, nrow = 2, rel_heights = c(0.85, 0.15))
...
\begin{figure*}[!t]
\centering
<<app-plot-step2a, out.width = '\\textwidth', fig.width = 8, fig.height = 2.25>>=
