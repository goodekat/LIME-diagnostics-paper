...
bullet_train$rfscore = predict(bullet_rf, bullet_train %>% select(bullet_features), type = "prob")[,2]
bullet_test$rfscore = predict(bullet_rf, bullet_test %>% select(bullet_features), type = "prob")[,2]
...
bullet_test_pred = predict(bullet_rf, bullet_test %>% select(bullet_features))
...
if (!file.exists("figure-static/figure-08-1.eps")) {
  unzip(zipfile = "figure-static/figure-08-1.png.zip", exdir = "figure-static")
...
pcp_file_name_png = "figure-static/figure-10-1.png"
pcp_file_name_eps = "figure-static/figure-10-1.eps"
...
%XXX minor change to emphasize that this is not a completely new model, just an expanded training set - otherwise you open yourself up to criticism of not using a validated model. The bulletxtrctr model was used in an external validation in \citet{vanderplas:2020b}. 

\kgc{Your comment about being clear that we are not training a whole new model made lots of sense. I made a few more changes. Do they look okay?} \kge{A random forest model fit to an expanded dataset of \Sexpr{format(dim(bullet_train)[1], big.mark = ",")} land-to-land comparisons from two sets of bullets in the James Hamby Consecutively Rifled Ruger Barrel Study \citep{hamby:2009} was validated in \citet{vanderplas:2020b}. Here, we train a random forest model to mimic the model validated in \citet{vanderplas:2020b} using the same data, model structure, and features (the nine similarity measures developed in \citet{hare:2017})}. %Here, we train a random forest model using the same \kge{data, features (the nine similarity measures developed in \citet{hare:2017}), and model structure as a random forest validated in \citet{vanderplas:2020b}} \kge{\sout{nine comparison features as \citet{hare:2017}}} to predict whether two lands come from the same source (i.e.\ are a match) in an expanded set of \Sexpr{format(dim(bullet_train)[1], big.mark = ",")} land-to-land comparisons from two sets of bullets in the James Hamby Consecutively Rifled Ruger Barrel Study \citep{hamby:2009}. 
We apply the trained random forest model to 6 bullets from another set of the Hamby study with \Sexpr{dim(bullet_test)[1]} rows of land comparisons. See the supporting information for further descriptions of the data. \kge{The newly trained random forest has an out-of-bag accuracy of \Sexpr{bullet_rf_oob_acc} and out-of-bag false positive and false negative rates of \Sexpr{bullet_rf_oob_fp} and \Sexpr{bullet_rf_oob_fn}, respectively. On the test data, the random forest has an accuracy of \Sexpr{bullet_rf_test_acc} and false positive and false negative rates of \Sexpr{bullet_rf_test_fp} and \Sexpr{bullet_rf_test_fn}, respectively.}
...
\kge{We \sout{First,}} focus on the explanation for case NM with 4-quantile-bins. The explanation plot from \emph{lime} shows that \kge{the} observed values \kge{of NM of \sout{with}} \kge{mismatches less than or equal to 7.17, rough correlation (rough\_cor) greater than 0.05, and D less than or equal to 0.002 support a random forest prediction in favor of a match. The scatter plots show that many of the simulated values in the rectangular regions with mismatches less than 7.17, rough correlation greater than 0.05, and distance less than or equal to 0.002 have random forest predictions greater than 0.5.} %rough correlation (rough\_cor) greater than 0.05 and mismatches value less than or equal to 7.17 support a random forest prediction in favor of a match, and the scatter plots show that many of the simulated values with rough correlation greater than 0.05 and mismatches less than 7.17 have random forest predictions greater than 0.5. Additionally, the LIME explanation indicates that a value of cross correlation function (ccf) greater than 0.28 and less than or equal to 0.35 supports a non-match, and the scatter plots shows that the simulated values within this region have mostly random forest predictions less than 0.5. 
However, the random forest assigns a probability of 0.21 to case NM, which is not explained by the LIME explanation \kge{that provides all explanations in support of a match}. Instead, consider that a better explanation based on the scatterplots would be that the random forest assigned a prediction probably \kge{below 0.5 \sout{close to 0}} since \kge{NM has observed values of mismatches less than 5, rough correlation greater than 0.25, and distance greater than 0.001.}%the observation has a rough correlation less than 0.5, mismatches greater than 4, and cross correlation function 0.6.

The other explanation scatterplots can be interpreted in a similar manner. \kge{\sout{Overall, t} T}he \kge{three plots depict situations where the explanations are faithful} %plots identify that the 4-quantile-bin explanations are not faithful to the random forest model predictions. The 4-equal-bin explanations are more faithful 
to the random forest, but the regions captured by the \kge{intersections of the} bins do not align well with the regions containing similar probabilities produced by the random forest.
