...
To demonstrate the visual diagnostics, we generate an example dataset that will be referred to as the \data. The \data \ contains \Sexpr{dim(sine_data)[1]} observations with \sout{three features and one response variable. The} features\sout{,} \kge{of} $x_1$, $x_2$, and $x_3$\sout{,} \kge{that} are randomly sampled from Unif(\Sexpr{min}, \Sexpr{max}), Unif(\Sexpr{min}, \Sexpr{max}), and $\mbox{N}(0,1)$ distributions, respectively. A binary response variable $y$ is created using a \sout{rotated} sine curve \sout{. In particular, let} \kge{such that}
