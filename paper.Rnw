...
To demonstrate the visual diagnostics, we generate an example dataset that will be referred to as the \data. The \data \ contains \Sexpr{dim(sine_data)[1]} observations with \sout{three features and one response variable. The} features\sout{,} \kge{of} $x_1$\sout{,} \kge{and} $x_2$ \kge{independently sampled from Unif(\Sexpr{min}, \Sexpr{max}) distributions}\sout{,} and $x_3$\sout{,} \sout{\kge{that} are randomly sampled from Unif(\Sexpr{min}, \Sexpr{max}), Unif(\Sexpr{min}, \Sexpr{max}), and} \kge{sampled from a} $\mbox{N}(0,1)$ distribution\sout{s, respectively}. A binary response variable $y$ is created using a \sout{rotated} sine curve \sout{. In particular, let} \kge{such that}
...
\begin{figure}[!tp]
<<figure-04, out.width = '3.125in', fig.width = 4, fig.height = 4.5>>=
...
<<figure-07, out.width = '3.125in', fig.width = 6, fig.height = 3.5, warning = FALSE>>=
...
\begin{figure*}[!thp]
<<figure-11, out.width = '6.25in', fig.width = 13, fig.height = 8.75, message = FALSE>>=
...
\begin{figure}[!thp]
<<figure-D4, out.width = '3.125in', fig.width = 6, fig.height = 3>>=

# Specify the figure size (for determining font size)
fd4_ow = 3.125
fd4_fw = 6
fd4_fs = fs * (fd4_fw / fd4_ow)
...
heatmap_plot_good <- ggplot(data = heatmap_data_good,
  ...
  theme_bw(base_family = ff, base_size = fd4_fs) +
  scale_fill_grey() +
  scale_color_grey() +
  theme(legend.position = "none", 
        plot.title = element_text(size = fd4_fs))
...
heatmap_plot_bad <- ggplot(data = heatmap_data_bad,
  ...
  theme_bw(base_family = ff, base_size = fd4_fs) +
  theme(plot.title = element_text(size = fd4_fs)) +
  scale_fill_grey() +
  scale_color_grey()
...
\begin{figure*}[!thp]
<<figure-e5, out.width = '6.25in', fig.width = 13, fig.height = 8.75, message = FALSE>>=

# Specify the figure size (for determining font size)
fe5_ow = 6.25
fe5_fw = 13
fe5_fs = fs * (fe5_fw / fe5_ow)
fe5_ls = 0.5 * (fe5_fw / fe5_ow)
