...
The \data \ explanation only includes two features. In situations were more than two features are included in a LIME explanation, the explanation scatterplots can be extended to a generalized scatterplot matrix \kgc{(reference)} that includes all pairwise combinations of features. Generalized scatterplot matrices (and scatterplot matrices in general) fail to provide helpful information when the number of features becomes large \kgc{(reference)}. It is common for machine learning models to have a large number of features, and therefore, a generalized scatterplot matrix of explanation scatterplots for all features would be ineffective. However, in the application of LIME, the user selects the number of features to return in the explanation (as previously mentioned). \citet{pedersen:2020} encourage users to select less than 10 features to include in the explanation\footnote{https://cran.r-project.org/web/packages/lime/vignettes/Understanding\_lime.html}. As a result, specifying a small number of features to include in a LIME explanation makes it feasible to use generalized scatterplot matrices of explanation scatterplots.
...
Each metric presented below is computed on a set of LIME explanations associated with a set predictions of interest obtained using the the same tuning parameters. Here we provide a high level description of the metrics. Notation and formulas for these metrics are included in Appendix \ref{metric-details}.
