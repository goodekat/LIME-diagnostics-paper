...
In this section, we introduce four visual diagnostic plots for the assessment of LIME. The plots focus on different levels of application of LIME (e.g. on explanation versus a set of explanations) to assess the LIME claims from different perspectives:

\begin{enumerate}
\item \emph{Explanation Scatterplot} (Section \ref{plot1}): Comparison of the explainer and complex models for an individual prediction of interest.
\item \emph{Prediction Comparison Plot} (Section \ref{plot2}): Comparison of explainer and complex model predictions for a set of predictions.
\item \emph{Feature Heatmap} (Section \ref{plot3}): Comparison of features selected by LIME across applications of LIME with different tuning parameters.k
\item \emph{Assessment Metric Plot}: Comparison of performance metrics for LIME across applications of LIME with different tuning parameters. 
\end{enumerate}
...
\subsection{Feature Heatmap Diagnostic Plot} \label{plot3}
...
\subsection{Assessment Metric Plot} \label{plot4}
