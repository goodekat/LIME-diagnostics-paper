...
#   3. add a variable with the random forest probability in support of a match
#   4. change the name of the same_source variable to match the test data
#   5. reorder the variables
bullet_train_temp <- read.csv("data-bullet-train.csv") %>%
  #filter(land_id1 == "Hamby173-BrUnk-B2-L1")
  select(-lag,-abs_lag,-signature_length) %>%
  mutate(
    case = factor(1:n()),
    rfscore = predict(rtrees, bullet_train_temp %>% select(rownames(rtrees$importance)), type = "prob")[, 2]
  ) %>%
