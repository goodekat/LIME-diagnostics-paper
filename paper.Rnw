...
# Create a path where the random forest will be saved
bullet_rf_file = "data/bullet-rf.rds"
if (file.exists(bullet_rf_file)) {
  # Load the random forest
  bullet_rf = readRDS(bullet_rf_file)
} else {
  # Fit the random forest to mimic rtrees
  set.seed(20160104)
  bullet_rf <-
    randomForest(
  ...
  saveRDS(bullet_rf, bullet_rf_file)
}
...
# Add a variables to training and testing data with random forest predictions
bullet_train$rfscore = predict(bullet_rf, type = "prob")[,2]
