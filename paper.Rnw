...
An explanation scatterplot corresponding to the LIME explanation depicted on the left in \autoref{fig:sine-exp-scatter} is shown on the right hand side of \autoref{fig:sine-exp-scatter}. By juxtaposing the random forest predictions and the explainer model boundaries, we are able to assess the faithfulness and localness of the explainer model. 
...
<<sine-feat-heat, out.width = '0.45\\textwidth', fig.width = 4, fig.height = 4.5>>=
