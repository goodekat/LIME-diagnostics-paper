...
pfM_3qb <- plot_features(bullet_explain_perms[1:3,]) +
  ...
  theme(text = element_text(family = ff, size = f13_fs),
...
\kge{\begin{itemize}
\item \textbf{Paper GitHub Repository}: The R markdown document and code used to produce the paper are available in the following GitHub Repository: \href{https://github.com/goodekat/LIME-diagnostics-paper-short-sweet}{https://github.com/goodekat/LIME-diagnostics-paper-short-sweet}. \kgc{Need to update this link after I change the name of the repository.}
\item \textbf{Data}: The data used for the examples in Section \ref{application} are available at \kgc{need to fill in once Heike has the data published.}
\end{itemize}}
