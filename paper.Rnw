...
The forked version \hh{is functionally indistinguishable from Pedersen's implementation}\kge{,} \hh{but it} allows us to export internal values \hh{relevant for an assessment of the explainer}. 
