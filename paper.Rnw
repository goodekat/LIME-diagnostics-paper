...
LIME \sout{is implemented using the R package \emph{limeaid} to apply LIME} \kge{is applied} \sout{multiple times} to all test set observations using different tuning parameters: 12 sampling methods (2-6 equally spaced bins, 2-6 quantile bins, kernel density estimation, and normal approximation) and 3 Gower exponents (0.5, 1, and 10). Thus, a total of $12\times 3=36$ different applications of LIME were performed. We specify that each LIME explanation return 3 features and feature selection is performed \kge{by selecting the features with the highest weights in a ridge regression model} \sout{using the highest weights method} (default option).
