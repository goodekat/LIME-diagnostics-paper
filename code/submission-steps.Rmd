...
2. Run the following code to move all files necessary to the submission folder.:
...
3. Manually delete `figure/` and `figure-static/` from the figure file paths in `paper.tex`. For example, change `\includegraphics[width=6.5in]{figure/figure-01-1}` to `\includegraphics[width=6.5in]{figure-01-1}`.

4. Compile the file `paper.tex` (using LaTex, then bibTex, then LaTeX twice more) to make sure it compiles correctly (especially references).

5. Besides for `paper.pdf`, delete all other files generated when `paper.tex` was compiled. This includes other paper files and pdf versions of the figures.

6. Upload all remaining files to ASA Data Science Journal submission page which should be:
