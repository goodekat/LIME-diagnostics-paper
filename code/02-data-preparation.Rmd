...
Load the newest training data provided by Heike to determine if the same data:

```{r}
ccfs_withlands_raw = read.csv("../data/raw/CCFs_withlands.csv")
```

Adjust the raw data to match the structure of `bullet_train_raw`: 
...
letters <- 
  ccfs_withlands_raw_expanded %>%
  filter(!(barrel1 %in% 1:10)) %>% 
  pull(barrel1) %>%
  unique()

letters2 <- 
  ccfs_withlands_raw_expanded %>%
  filter(!(barrel2 %in% 1:10)) %>% 
  pull(barrel2) %>%
  unique()
sum(letters != letters2) > 0

ccfs_withlands_expanded <- ccfs_withlands_raw_expanded %>%
  mutate(
...
The labels agree now but note that :
...
plot_grid(
  bullet_train_raw_expanded %>% count(study1, study2) %>%
    ...
    theme(aspect.ratio = 1) + 
    labs(title = "Hamby comparisons data"),
  ccfs_withlands_expanded %>% count(study1, study2) %>%
    ggplot(aes(
      x = study1, y = study2, fill = n
    )) +
    geom_tile() +
...
plot_grid(
  bullet_train_raw_expanded %>% count(barrel1, barrel2) %>%
    ...
    theme(aspect.ratio = 1) + 
    labs(title = "Hamby comparisons data"),
  ccfs_withlands_expanded %>% count(barrel1, barrel2) %>%
    ggplot(aes(
      x = barrel1, y = barrel2, fill = n
    )) +
    geom_tile() +
...
plot_grid(
  bullet_train_raw_expanded %>% count(bullet1, bullet2) %>%
    ...
    theme(aspect.ratio = 1) + 
    labs(title = "Hamby comparisons data"),
  ccfs_withlands_expanded %>% count(bullet1, bullet2) %>%
    ggplot(aes(
      x = bullet1, y = bullet2, fill = n
    )) +
    geom_tile() +
...
plot_grid(
  bullet_train_raw_expanded %>% count(land1, land2) %>%
    ...
    theme(aspect.ratio = 1) + 
    labs(title = "Hamby comparisons data"),
  ccfs_withlands_expanded %>% count(land1, land2) %>%
    ggplot(aes(
      x = land1, y = land2, fill = n
    )) +
    geom_tile() +
...
plot_grid(
  bullet_train_raw_expanded %>% 
    mutate(across(where(is.character), factor)) %>%
    filter(study1 == "Hamby173", study2 == "Hamby173") %>%
    filter(bullet1 %in% c("B1","B2"), bullet2 %in% c("B1", "B2")) %>% 
    count(study1, barrel1, bullet1, land1, study2, barrel2, bullet2, land2) %>%
    ggplot(aes(
      x = barrel1:land1, y = barrel2:land2, fill = n
    )) +
    geom_tile() +
    facet_grid(study2 + bullet2 ~ study1 + bullet1) +
    theme(aspect.ratio = 1,
...
```{r}
bullet_train_raw_switch <-
  bullet_train_raw_expanded %>%
  mutate(across(where(is.factor), as.character)) %>%
  mutate(temp_barrel = ifelse(barrel1 == "Br10", barrel1, "NA")) %>%
  mutate(barrel1 = ifelse(temp_barrel == "Br10", barrel2, barrel1)) %>%
  mutate(barrel2 = ifelse(temp_barrel == "Br10", temp_barrel, barrel2)) %>%
  select(-temp_barrel) %>%
  mutate(across(where(is.character), factor)) %>%
  mutate(barrel1 = fct_relevel(barrel1, paste0("Br", 1:10)),
         barrel2 = fct_relevel(barrel2, paste0("Br", 1:10)))
...
```{r fig.width = 15, fig.height = 30, echo = FALSE}
plot_grid(
  bullet_train_raw_switch %>% 
    filter(study1 == "Hamby173", study2 == "Hamby173") %>%
    filter(bullet1 %in% c("B1","B2"), bullet2 %in% c("B1", "B2")) %>% 
    count(study1, barrel1, bullet1, study2, barrel2, bullet2) %>%
    ggplot(aes(
      x = barrel1, y = barrel2, fill = n
    )) +
    geom_tile() +
  ...
  ccfs_withlands_expanded %>% 
    ...
    labs(title = "CCFs with lands data"), 
bullet_train_raw_switch %>% 
    filter(study1 == "Hamby173", study2 == "Hamby252") %>%
    filter(bullet1 %in% c("B1","B2"), bullet2 %in% c("B1", "B2")) %>% 
    count(study1, barrel1, bullet1, study2, barrel2, bullet2) %>%
    ggplot(aes(
      x = barrel1, y = barrel2, fill = n
  ...
  ccfs_withlands_expanded %>% 
    ...
    labs(title = "CCFs with lands data"),
bullet_train_raw_switch %>% 
    filter(study1 == "Hamby252", study2 == "Hamby173") %>%
    filter(bullet1 %in% c("B1","B2"), bullet2 %in% c("B1", "B2")) %>% 
    count(study1, barrel1, bullet1, study2, barrel2, bullet2) %>%
    ggplot(aes(
      x = barrel1, y = barrel2, fill = n
...
```{r}
bullet_train_cleaned <-
  bullet_train_raw_switch %>%
  mutate(
    land_id1 = paste(study1, barrel1, bullet1, land1, sep = "-"),
    land_id2 = paste(study2, barrel2, bullet2, land2, sep = "-")
  ) %>%
  select(land_id1, land_id2, rf_features)

ccfs_withlands_cleaned <- ccfs_withlands_expanded %>%
  mutate(
    land_id1 = paste(study1, barrel1, bullet1, land1, sep = "-"),
    land_id2 = paste(study2, barrel2, bullet2, land2, sep = "-")
  ) %>%
...
```{r}
land_ids_old = bullet_train_cleaned %>% select(land_id1, land_id2)
land_ids_new = ccfs_withlands_cleaned %>% select(land_id1, land_id2)
dim(land_ids_old)
dim(land_ids_new)
```
...
```{r}
extra_leas_in_old <- 
  anti_join(x = land_ids_old, y = land_ids_new, by = c("land_id1", "land_id2")) %>%
  separate(land_id1, c("study1", "barrel1", "bullet1", "land1")) %>%
  separate(land_id2, c("study2", "barrel2", "bullet2", "land2"))
dim(extra_leas_in_old)
```

```{r fig.height = 6, echo = FALSE}
extra_leas_in_old %>%
  count(study1, barrel1, study2, barrel2) %>%
  ggplot(aes(x = barrel1, y = barrel2, fill = n)) +
  geom_tile() +
  facet_grid(study2 ~ study1, scales = "free") + 
  theme(aspect.ratio = 1)
```

```{r}
extra_leas_in_new <- 
  anti_join(x = land_ids_new, y = land_ids_old, by = c("land_id1", "land_id2")) %>%
  separate(land_id1, c("study1", "barrel1", "bullet1", "land1")) %>%
  separate(land_id2, c("study2", "barrel2", "bullet2", "land2"))
dim(extra_leas_in_new)
```
