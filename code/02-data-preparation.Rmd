...
new_pred = predict(bulletxtrctr::rtrees, bullet_train_filtered %>% select(rownames(bulletxtrctr::rtrees$importance)), type = "prob")[,2]
new_pred_full = predict(bulletxtrctr::rtrees, bullet_train_raw %>% select(rownames(bulletxtrctr::rtrees$importance)), type = "prob")[,2]
```

Plot comparing the two predictions:
...
Plot showing distributions of predictions from `rtrees` and new predictions made on full training data:

```{r}
data.frame(data = c(rep("rtrees", length(rtrees_pred)), rep("new predictions on full data", length(new_pred_full))), pred = c(rtrees_pred, new_pred_full)) %>%
